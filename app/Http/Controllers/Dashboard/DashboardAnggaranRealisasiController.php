...
class DashboardAnggaranRealisasiController extends Controller
{
    ...
    public function showAnggaranDanRealisasi()
    {
        ...
        $baru = AnggaranRealisasi::with(['kecamatan'])->first();
        $baru['realisasi'] = ($baru['total_belanja'] / $baru['total_anggaran']) * 100;
        $baru['r_pegawai'] = ($baru['belanja_pegawai'] / $baru['total_anggaran']) * 100;
        $baru['r_jasa'] = ($baru['belanja_barang_jasa'] / $baru['total_anggaran']) * 100;
        $baru['r_modal'] = ($baru['belanja_modal'] / $baru['total_anggaran']) * 100;
        $baru['r_btl'] = ($baru['belanja_tidak_langsung'] / $baru['total_anggaran']) * 100;
        return view('dashboard.anggaran_realisasi.show_anggaran_realisasi', compact('baru'))->with($data);
    }
